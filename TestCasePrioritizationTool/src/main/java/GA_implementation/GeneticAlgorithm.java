...
public class GeneticAlgorithm {
	 ...
	 */
    public double calcFitness(Individual individual, int[][] coverage_matrix, int num_rules, int num_test_cases){
        // Get fitness
        ...
        for(int i=0; i< num_rules; i++) {
        	...
        	for(j=0; j< num_test_cases; j++) {
        		if(coverage_matrix[i][individual.getChromosome()[j]]==1) break;
        	}
        	TR[i] = j+1;
        }
        ...
        for(int i = 0; i< num_rules; i++) 
        	TR_sum += TR[i];
        double APRD = 100 * (1 - (double) TR_sum/(num_rules * num_test_cases) + 1.0/(2*num_test_cases));
    	...
    	for(j=0; j<num_test_cases; j++) {
    		for(int i=0; i<num_rules; i++) {
    			if(coverage_matrix[i][individual.getChromosome()[j]]==1) TR[i]=1;
    		}
    		...
    		if(x==num_rules) break;
    	}
    	
        double fitness = 1.0/(j+1);
        individual.setFitness(fitness);*/
     ...
     */
    public void evalPopulation(Population population, int[][] coverage_matrix, int num_rules, int num_test_cases){
        double populationFitness = 0;
        
        // Loop over population evaluating individuals and summing population fitness
        for (Individual individual : population.getIndividuals()) {
            populationFitness += this.calcFitness(individual, coverage_matrix, num_rules, num_test_cases);
        }
